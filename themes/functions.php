...
function get_debug() {
  ...
  if(isset($ly->config['debug']['leroy']) && $ly->config['debug']['leroy']) {
  ...
  if(isset($ly->config['debug']['session']) && $ly->config['debug']['session']) {
    $html .= "<hr><h3>SESSION</h3><p>The content of CLeroy->session:</p><pre>" . htmlent(print_r($ly->session, true)) . "</pre>";
...
function get_messages_from_session() {
  $messages = CLeroy::Instance()->session->GetMessages();
  $html = null;
  if(!empty($messages)) {
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CLeroy::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function render_views($region='default') {
  return CLeroy::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CLeroy::Instance()->views->RegionHasView(func_get_args());
}
